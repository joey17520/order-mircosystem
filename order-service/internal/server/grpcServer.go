...
import (
	...
	"order-microsystem/order-service/pkg/monitoring"
...
func (s *GRPCServer) Start(orderController *controller.OrderController) error {
	listen, err := net.Listen("tcp", fmt.Sprintf("%s:%d", s.config.Server.Host, s.config.Server.Port))
	if err != nil {
		return fmt.Errorf("failed to listen: %v", err)
	}

	controller.RegisterOrderService(s.server, orderController)
...
func (s *GRPCServer) RegisterWithConsul() error {
	...
	if err != nil {
		return fmt.Errorf("failed to create Consul client after retries: %v", err)
	}
	s.consul = client
