...
import (
	...
	"order-microsystem/payment-service/pkg/monitoring"
...
func (s *GRPCServer) Start(inventorycontroller *controller.PaymentController) error {
	listen, err := net.Listen("tcp", fmt.Sprintf("%s:%d", s.config.Server.Host, s.config.Server.Port))
	if err != nil {
		return fmt.Errorf("failed to listen: %v", err)
	}

	controller.RegisterPaymentController(s.server, inventorycontroller)
